...
from api.auth import OptionalUser, apply_duplicate_works_scope, verify_work_jurisdiction
...
def list_duplicate_works(
    ...
    if state or district:
        ...
        if state:
            query = query.filter(or_(func.upper(w1.state) == state.upper(), func.upper(w2.state) == state.upper()))
        if district:
            query = query.filter(or_(func.upper(w1.district) == district.upper(), func.upper(w2.district) == district.upper()))
